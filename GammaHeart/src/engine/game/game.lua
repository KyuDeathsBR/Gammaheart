...
---@param ... string|PartyAlly
function Game:setPartyAllies(...)
    local args = {...}
    self.ally = {}
    for i,chara in ipairs(args) do
        if type(chara) == "string" then
            self.ally[i] = self:getPartyAlly(chara)
        else
            self.ally[i] = chara
        end
    end
...
function Game:load(data, index, fade)
    ...
    for _,id in ipairs(data.party or Kristal.getModOption("party") or {"kris"}) do
        local party = self:getPartyMember(id)
        if party then
            table.insert(self.party, party)
        else
    ...
    for _,id in ipairs(data.ally or Kristal.getModOption("allies") or {}) do
        ...
        if ally then
            ...
            Kristal.Console:error("Could not load party ally \"" ..id.."\"")
        end
    end
    ...
    for i,party in next, self.party_data, nil do
        local character = Game.world:getPartyCharacterInParty(party)
        if character then
            party.mode = party.mode or Mod and Mod.mode or data.mode
            party:onModeChange(nil,nil,character)
        end
    end
    
    if data.language then
        Mod.selected_language = data.language
    end
end
