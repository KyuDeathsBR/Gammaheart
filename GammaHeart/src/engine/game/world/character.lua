...
---Returns the party member, if there is one.
---@return PartyMember?
...
function Character:update()
    ...
    ---@type PartyMember|PartyAlly?
    local party_member = self:getPartyMember() or self:getPartyAlly()
    if party_member then
        if party_member:includes(PartyAlly) then
            if party_member:getAmulet() then
                party_member:getAmulet():onWorldUpdate(self)
            end
        else
