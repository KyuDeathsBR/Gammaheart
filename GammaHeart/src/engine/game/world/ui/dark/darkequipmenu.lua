...
function DarkEquipMenu:updateDescription()
    ...
    elseif self.state == "SLOTS" then
        ...
        local item
        if self.selected_slot == 1 then
            if party:includes(PartyAlly) then
                item = party:getAmulet()
            else
                item = party:includes(PartyAlly) and party:getAmulet() or party:getWeapon()
            end
        else
...
function DarkEquipMenu:drawEquipped()
    ...
    if self.state ~= "SLOTS" or self.selected_slot ~= 1 then
        local weapon_icon = Assets.getTexture(party:includes(PartyAlly) and party:getAmuletIcon() or party:getWeaponIcon())
        if weapon_icon then
...
function DarkEquipMenu:drawEquippedItem(index, x, y)
    ...
    local item
    if index == 1 then
        if party:includes(PartyMember) then
            item = party:getWeapon()
        elseif party:includes(PartyAlly) then
            item = party:getAmulet()
        end
    else
