...
function DarkMenuPartySelect:getSelected()
    return self.selected_party <= #Game.party and Game.party[self.selected_party] or Game.ally[self.selected_party-#Game.party]
end

function DarkMenuPartySelect:updateSelectedParty()
    self.selected_party = (self.selected_party - 1) % (#Game.party + #Game.ally) + 1
end
...
function DarkMenuPartySelect:draw()
    if self.selected_party <= #Game.party then
        ...
        for i,party in ipairs(Game.ally) do
            if math.ceil(i/3) == math.ceil(self.selected_party/3) then
                if self.selected_party ~= i then
                Draw.setColor(1, 1, 1, 0.4)
                else
                    Draw.setColor(1, 1, 1, 1)
                ...
                local ox, oy = party:getMenuIconOffset()
                Draw.draw(Assets.getTexture(party:getMenuIcon()), (i-1)%3*50 + (ox*2), oy*2, 0, 2, 2)
            end
        end
        if self.focused then
            local frames = Assets.getFrames("player/heart_harrows")
            Draw.setColor(Game:getSoulColor())
            Draw.draw(frames[(math.floor(self.heart_siner/20)-1)%#frames+1], (self.selected_party-1-#Game.party)%3*50 + 10, -18)
        end
    end
    super.draw(self)
end
