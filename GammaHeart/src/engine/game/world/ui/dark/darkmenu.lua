    ...
    elseif self.state == "PARTYSELECT" then
        ...
        if self.party_select_mode == "SINGLE" then
            ...
            if Input.is("up", key) then
                self.selected_party = self.selected_party - math.max(#Game.party,#Game.ally)
                self.ui_move:stop()
                self.ui_move:play()
            end
            if Input.is("down", key) then
                self.selected_party = self.selected_party + math.min(#Game.party,#Game.ally)
                self.ui_move:stop()
                self.ui_move:play()
            end
        end
        if self.selected_party < 1 then self.selected_party = #Game.party + #Game.ally end
        if self.selected_party > #Game.party + #Game.ally then self.selected_party = 1 end
...
function DarkMenu:draw()
    ...
    local currency = Game:getConfig("darkCurrencyShort")
    if type(currency) == "table" then
        currency = currency[Mod.selected_language] or currency.default or currency[1]
    end
    love.graphics.print(currency .. " " .. Game.money, 520, 20)
