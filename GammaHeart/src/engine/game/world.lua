...
---@param party_member string|PartyMember|PartyAlly
...
function World:getActionBox(party_member)
    ...
    if type(party_member) == "string" then
        party_member = Game:getPartyMember(party_member) or Game:getPartyAlly(party_member)
    end
...
function World:loadMap(...)
    ...
    if marker then
        self:spawnParty(marker, team, nil, facing)
    else
        self:spawnParty({x, y}, team, nil, facing)
    end
