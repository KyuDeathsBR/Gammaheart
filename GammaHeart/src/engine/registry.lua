...
---@field party_allies table<string, PartyAlly>
...
---*ALIAS* Returns an [ally](lua://PartyAlly) in the registry if possible. this is an alias of [Registry.getPartyAlly](lua://Registry.getPartyAlly)
---@param id string
---@return PartyAlly|nil
function Registry.getAlly(id)
    return Registry.getPartyAlly(id)
end

---Returns an [ally](lua://PartyAlly) in the registry if possible.
---@param id string
---@return PartyAlly|nil
function Registry.getPartyAlly(id)
...
---@param id string
---@param class PartyAlly
function Registry.registerAlly(id, class)
    self.registerPartyAlly(id,class)
end

---@param id string
---@param class PartyAlly
function Registry.registerPartyAlly(id, class)
    self.party_allies[id] = class
end
...
function Registry.initAllies()
    self.party_allies = {}

    for _,path,char in self.iterScripts(Registry.paths["allies"]) do
        assert(char ~= nil, '"battle/allies/'..path..'.lua" does not return value')
        char.id = char.id or path
        self.registerAlly(char.id, char)
    end

    Kristal.callEvent(KRISTAL_EVENT.onRegisterAllies)
end
