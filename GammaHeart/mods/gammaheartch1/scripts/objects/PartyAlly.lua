...
---@field title string
---@field level integer
---@field unique_id string? Unique ID generated by the code, it follows the rule "`id`$`number of party members at the time`#`os clock value`".
---@field lw_lv integer
---@field lw_xp number
---
...
function PartyAlly:init(unique_id)
...
function PartyAlly:convertToLight()
    ...
    if not self.equipped.amulet then
        self.equipped.amulet = Registry.createItem(self.lw_weapon_default)
    end
    if not self.equipped.armor[1] then
        self.equipped.armor[1] = Registry.createItem(self.lw_armor_default)
    end

    self.equipped.amulet.dark_item = last_weapon
...
function PartyAlly:saveEquipment()
    ...
    if self.equipped.amulet then
        result.weapon = self.equipped.amulet:save()
    end
    for i = 1, 2 do
        if self.equipped.armor[i] then
...
function PartyAlly:loadEquipment(data)
    self:setAmulet(nil)
    if data.amulet then
        if type(data.amulet) == "table" then
            if Registry.getItem(data.amulet.id) then
                local weapon = Registry.createItem(data.amulet.id)
                if weapon then
                    weapon:load(data.amulet)
                    self:setAmulet(weapon)
                else
                    Kristal.Console:error("Could not load weapon \""..data.amulet.id.."\"")
                end
            else
                Kristal.Console:error("Could not load weapon \"".. data.amulet.id .."\"")
            end
        else
            if Registry.getItem(data.amulet) then
                self:setAmulet(data.amulet)
            else
                Kristal.Console:error("Could not load weapon \"".. (data.amulet or "nil") .."\"")
            end
        end
    end
    for i = 1, 2 do
...
function PartyAlly:load(data)
    ...
    self.unique_id = data.uniqueid or self.unique_id
