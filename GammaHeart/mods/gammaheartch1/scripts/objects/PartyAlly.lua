---@source local/GammaHeart/mods/chselect/scripts/objects/MAIN/CHAPTER_SELECT.lua
...
---@field lw_xp number
---
---@field priority_type "TARGET_ONLY"? it has only this option currently
...
---
---@field amulet_icon string
---
---@field equipped {amulet: Item?, armor: Item[]}
---
...
---@field dialogue {[LanguageID]:string[]|string}
...
---
---@overload fun(...) : PartyAlly
local PartyAlly = Class()

function PartyAlly:init()
    self.mode = "DARK"
    -- Display name
    self.name = {
        en = "Ally",
    ...
    self.amulet_icon = "ui/menu/equip/scarf"
...
---@param damage number
function PartyAlly:onAttackHit(enemy, damage) end
...
function PartyAlly:onTurnStart(battler) end
...
---@param menu DarkPowerMenu The current menu instance
function PartyAlly:drawPowerStat(index, x, y, menu) end

--- *(Override)* Called whenever the party member's data is saved
---@param data PartyAllySaveData
function PartyAlly:onSave(data) end
--- *(Override)* Called whenever the party member's data is loaded
---@param data PartyAllySaveData
function PartyAlly:onLoad(data) end
...
function PartyAlly:onEquip(item, item2) return true end
...
function PartyAlly:onUnequip(item, item2) return true end

function PartyAlly:onActionBox(box, overworld) end

-- Getters

function PartyAlly:getLevel() return self.level end

function PartyAlly:getLightLV() return self.lw_lv end
function PartyAlly:getLightEXP() return self.lw_exp end
function PartyAlly:getLightEXPNeeded(lv) return self.lw_exp_needed[lv] or 0 end

function PartyAlly:getSoulPriority() return self.soul_priority end
function PartyAlly:getSoulColor() return Utils.unpackColor(self.soul_color or {1, 0, 0}) end

function PartyAlly:hasAct() return self.has_act end
function PartyAlly:hasSpells() return (self.mode == "DARK" or Mod.mode == "DARK") and self.has_spells or false end
function PartyAlly:hasXAct() return self.has_xact end

function PartyAlly:getXActName() return self.xact_name end

function PartyAlly:getAmuletIcon() return self.amulet_icon end

function PartyAlly:getHealth() return Game:isLight() and self.lw_health or self.health end
---@param light? boolean
function PartyAlly:getBaseStats(light)
...
function PartyAlly:getMaxStats() return self.max_stats end
---@param stat string
function PartyAlly:getMaxStat(stat)
    local max_stats = self:getMaxStats()
    return max_stats[stat]
end

function PartyAlly:getStrongerAbsent() return self.stronger_absent end

function PartyAlly:getStatBuffs() return self.stat_buffs end
---@param stat string
function PartyAlly:getStatBuff(stat)
    return self:getStatBuffs()[stat] or 0
end

function PartyAlly:getColor() return Utils.unpackColor(self.color) end
function PartyAlly:getDamageColor()
...
function PartyAlly:getAttackBarColor()
...
function PartyAlly:getAttackBoxColor()
...
function PartyAlly:getXActColor()
...
function PartyAlly:getMenuIcon() return self.menu_icon end
function PartyAlly:getHeadIcons() return self.head_icons end
function PartyAlly:getNameSprite() return self.name_sprite end

function PartyAlly:getAttackSprite() return self.attack_sprite end
function PartyAlly:getAttackSound() return self.attack_sound end
function PartyAlly:getAttackPitch() return self.attack_pitch end

---@return number x
---@return number y
function PartyAlly:getBattleOffset() return unpack(self.battle_offset or {0, 0}) end
---@return number x
---@return number y
function PartyAlly:getHeadIconOffset() return unpack(self.head_icon_offset or {0, 0}) end
---@return number x
---@return number y
function PartyAlly:getMenuIconOffset() return unpack(self.menu_icon_offset or {0, 0}) end

function PartyAlly:getGameOverMessage() return self.gameover_message end
...
function PartyAlly:heal(amount, playsound)
...
---@param health number
function PartyAlly:setHealth(health)
...
---@param amount    number
---@param max?      number
function PartyAlly:increaseStat(stat, amount, max)
...
---@param amount    number
---@param max?      number
function PartyAlly:addStatBuff(stat, amount, max)
...
---@param item Item
---@param user PartyAlly
---@return string?
function PartyAlly:getReaction(item, user)
    if item then
...
---@return Actor
function PartyAlly:getActor(force_mode)
...
function PartyAlly:onModeChange(new,old,character)
    if character then
        character:setActor(self:getActor())
    end
end

function PartyAlly:getName()
    if type(self.name) == "table" then
        return self.name[Mod.selected_language] or self.name[1]
    end
    return tostring(self.name)
end

function PartyAlly:getTitle()
    if type(self.title) == "table" then
        return self.title[Mod.selected_language] or ("LV"..self:getLevel().." "..(type(self.title[1]) == "table" and self.title[1] or self.title))
    end
    return "LV"..self:getLevel().." "..tostring(self.title)
end

function PartyAlly:hasSpells() return (self.mode == "DARK" or Mod.mode == "DARK") and self.has_spells or false end

---@return Actor
function PartyAlly:getDarkTransitionActor()
    return self.dark_transition_actor
end

--- Changes this party member's Dark World actor
---@param actor string|Actor
function PartyAlly:setActor(actor)
...
---@param actor string|Actor
function PartyAlly:setLightActor(actor)
    if type(actor) == "string" then
        actor = Registry.createActor(actor)
    end
    self.lw_actor = actor
...
---@param actor string|Actor
function PartyAlly:setDarkTransitionActor(actor)
    if type(actor) == "string" then
        actor = Registry.createActor(actor)
    end
...
---@param spell string|Spell
function PartyAlly:removeSpell(spell)
...
function PartyAlly:hasSpell(spell)
    for i,v in ipairs(self.spells) do
        if v == spell or (type(spell) == "string" and v.id == spell) then
            return true
        end
...
function PartyAlly:replaceSpell(spell, replacement)
    local tempspells = {}
...
---@return Item[]
function PartyAlly:getEquipment()
    local result = {}
    if self.equipped.amulet then
        table.insert(result, self.equipped.amulet)
    end
    for i = 1, 2 do
...
function PartyAlly:getAmulet()
    return self.equipped.amulet
end

---@param i integer
function PartyAlly:getArmor(i)
    return self.equipped.armor and self.equipped.armor[i]
end

---@param item string|Item
function PartyAlly:setAmulet(item)
    if type(item) == "string" then
        item = Registry.createItem(item)
    end
    self.equipped.amulet = item
end

---@param i     integer
---@param item  string|Item
function PartyAlly:setArmor(i, item)
    if type(item) == "string" then
        item = Registry.createItem(item)
    end
...
function PartyAlly:checkAmulet(id)
    return self:getAmulet() and self:getAmulet().id == id or false
end
...
function PartyAlly:checkArmor(id)
...
---@return boolean
function PartyAlly:canEquip(item, slot_type, slot_index)
    if item then
...
---@param stat string
---@return number
function PartyAlly:getEquipmentBonus(stat)
    local total = 0
...
---@param light? boolean
function PartyAlly:getStats(light)
...
---@param light?    boolean
function PartyAlly:getStat(name, default, light)
...
---@param default?  number
---@param light?    boolean
function PartyAlly:getBaseStat(name, default, light)
...
function PartyAlly:getFlag(name, default)
...
---@param flag  string
---@param value any
function PartyAlly:setFlag(flag, value)
...
function PartyAlly:addFlag(name, amount)
    self.flags[name] = (self.flags[name] or 0) + (amount or 1)
    return self.flags[name]
end

function PartyAlly:convertToLight()
    local last_weapon = self:getAmulet()
...
function PartyAlly:convertToDark()
...
function PartyAlly:saveEquipment()
...
---@param data table
function PartyAlly:loadEquipment(data)
    self:setWeapon(nil)
    if data.weapon then
...
function PartyAlly:loadSpells(data)
    self.spells = {}
...
---@return PartyAllySaveData
function PartyAlly:save()
    local data = {
        id = self.id,
        title = self.title,
        level = self.level,
...
function PartyAlly:canDeepCopy()
    return false
end
